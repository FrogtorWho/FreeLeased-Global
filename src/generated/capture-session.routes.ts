...
/**
 * Auto-generated CaptureSession Routes
 *
...
import type { CaptureSessionHooks } from "./capture-session.hooks"
...
/**
 * Set hooks for CaptureSession routes
 */
export function setCaptureSessionHooks(h: CaptureSessionHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["token", "status", "deviceType", "imageUrl", "ocrText", "docType", "confidence", "metadata", "expiresAt", "createdAt", "updatedAt"] as const
...
/**
 * Create CaptureSession CRUD routes
 */
export function createCaptureSessionRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[CaptureSession] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.captureSession.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "CaptureSession not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[CaptureSession] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.captureSession.create({
        data: picked.data,
      })
      ...
      console.error("[CaptureSession] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.captureSession.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[CaptureSession] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[CaptureSession] Delete error:", error)
