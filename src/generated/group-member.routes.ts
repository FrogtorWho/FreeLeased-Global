...
/**
 * Auto-generated GroupMember Routes
 *
...
import type { GroupMemberHooks } from "./group-member.hooks"
...
/**
 * Set hooks for GroupMember routes
 */
export function setGroupMemberHooks(h: GroupMemberHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "userId", "role", "displayName", "joinedAt"] as const
...
/**
 * Create GroupMember CRUD routes
 */
export function createGroupMemberRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[GroupMember] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupMember.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupMember not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupMember] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupMember.create({
        data: picked.data,
      })
      ...
      console.error("[GroupMember] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[GroupMember] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[GroupMember] Delete error:", error)
