...
/**
 * Auto-generated ResearchTask Routes
 *
...
import type { ResearchTaskHooks } from "./research-task.hooks"
...
/**
 * Set hooks for ResearchTask routes
 */
export function setResearchTaskHooks(h: ResearchTaskHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["kind", "target", "jurisdictionCode", "status", "rationale", "officialSources", "checklist", "findings", "assignedAgent", "createdAt", "updatedAt"] as const
...
/**
 * Create ResearchTask CRUD routes
 */
export function createResearchTaskRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ResearchTask] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.researchTask.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ResearchTask not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ResearchTask] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.researchTask.create({
        data: picked.data,
      })
      ...
      console.error("[ResearchTask] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.researchTask.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ResearchTask] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ResearchTask] Delete error:", error)
