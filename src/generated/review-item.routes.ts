...
/**
 * Auto-generated ReviewItem Routes
 *
...
import type { ReviewItemHooks } from "./review-item.hooks"
...
/**
 * Set hooks for ReviewItem routes
 */
export function setReviewItemHooks(h: ReviewItemHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["kind", "title", "claim", "jurisdiction", "residentId", "evidenceClass", "confidence", "sources", "agentTrail", "routing", "status", "reviewer", "annotation", "decidedAt", "rowHash", "appealReason", "appealedAt", "createdAt", "updatedAt"] as const
...
/**
 * Create ReviewItem CRUD routes
 */
export function createReviewItemRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ReviewItem] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ReviewItem not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ReviewItem] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      console.error("[ReviewItem] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      console.error("[ReviewItem] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ReviewItem] Delete error:", error)
