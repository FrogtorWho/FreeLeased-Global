...
/**
 * Auto-generated ResidentGroup Routes
 *
...
import type { ResidentGroupHooks } from "./resident-group.hooks"
...
/**
 * Set hooks for ResidentGroup routes
 */
export function setResidentGroupHooks(h: ResidentGroupHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "buildingName", "jurisdictionCode", "goal", "status", "description", "isPublic", "memberCount", "qualifyingThreshold", "createdAt", "updatedAt"] as const
...
/**
 * Create ResidentGroup CRUD routes
 */
export function createResidentGroupRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.residentGroup.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[ResidentGroup] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.residentGroup.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ResidentGroup not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ResidentGroup] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.residentGroup.create({
        data: picked.data,
      })
      ...
      console.error("[ResidentGroup] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.residentGroup.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ResidentGroup] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.residentGroup.delete({
        where: { id },
      })
      ...
      console.error("[ResidentGroup] Delete error:", error)
