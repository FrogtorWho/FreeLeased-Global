...
/**
 * Auto-generated ContentDraft Routes
 *
...
import type { ContentDraftHooks } from "./content-draft.hooks"
...
/**
 * Set hooks for ContentDraft routes
 */
export function setContentDraftHooks(h: ContentDraftHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["kind", "title", "body", "channel", "status", "metadata", "approvedAt", "postedAt", "createdAt", "updatedAt"] as const
...
/**
 * Create ContentDraft CRUD routes
 */
export function createContentDraftRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ContentDraft] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.contentDraft.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ContentDraft not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ContentDraft] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.contentDraft.create({
        data: picked.data,
      })
      ...
      console.error("[ContentDraft] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.contentDraft.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ContentDraft] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ContentDraft] Delete error:", error)
