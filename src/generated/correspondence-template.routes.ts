...
/**
 * Auto-generated CorrespondenceTemplate Routes
 *
...
import type { CorrespondenceTemplateHooks } from "./correspondence-template.hooks"
...
let hooks: CorrespondenceTemplateHooks = {}
...
/**
 * Set hooks for CorrespondenceTemplate routes
 */
export function setCorrespondenceTemplateHooks(h: CorrespondenceTemplateHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "jurisdictionCode", "category", "subject", "body", "legalRefs", "variables", "tier", "isOss", "createdAt", "updatedAt"] as const
...
/**
 * Create CorrespondenceTemplate CRUD routes
 */
export function createCorrespondenceTemplateRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.correspondenceTemplate.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[CorrespondenceTemplate] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.correspondenceTemplate.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "CorrespondenceTemplate not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[CorrespondenceTemplate] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.correspondenceTemplate.create({
        data: picked.data,
      })
      ...
      console.error("[CorrespondenceTemplate] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.correspondenceTemplate.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[CorrespondenceTemplate] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.correspondenceTemplate.delete({
        where: { id },
      })
      ...
      console.error("[CorrespondenceTemplate] Delete error:", error)
