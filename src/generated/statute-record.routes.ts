...
/**
 * Auto-generated StatuteRecord Routes
 *
...
import type { StatuteRecordHooks } from "./statute-record.hooks"
...
/**
 * Set hooks for StatuteRecord routes
 */
export function setStatuteRecordHooks(h: StatuteRecordHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["jurisdictionCode", "shortTitle", "citation", "url", "covers", "status", "conviction", "provenance", "lastReviewed", "createdAt"] as const
...
/**
 * Create StatuteRecord CRUD routes
 */
export function createStatuteRecordRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.statuteRecord.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[StatuteRecord] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.statuteRecord.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "StatuteRecord not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[StatuteRecord] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.statuteRecord.create({
        data: picked.data,
      })
      ...
      console.error("[StatuteRecord] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.statuteRecord.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[StatuteRecord] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.statuteRecord.delete({
        where: { id },
      })
      ...
      console.error("[StatuteRecord] Delete error:", error)
