...
/**
 * Auto-generated SignedDocument Routes
 *
...
import type { SignedDocumentHooks } from "./signed-document.hooks"
...
/**
 * Set hooks for SignedDocument routes
 */
export function setSignedDocumentHooks(h: SignedDocumentHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "templateId", "title", "body", "jurisdictionCode", "status", "requiredSigs", "collectedSigs", "sentAt", "createdAt", "updatedAt"] as const
...
/**
 * Create SignedDocument CRUD routes
 */
export function createSignedDocumentRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[SignedDocument] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.signedDocument.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "SignedDocument not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[SignedDocument] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.signedDocument.create({
        data: picked.data,
      })
      ...
      console.error("[SignedDocument] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.signedDocument.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[SignedDocument] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[SignedDocument] Delete error:", error)
