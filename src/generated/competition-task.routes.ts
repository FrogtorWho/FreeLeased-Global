...
/**
 * Auto-generated CompetitionTask Routes
 *
...
import type { CompetitionTaskHooks } from "./competition-task.hooks"
...
let hooks: CompetitionTaskHooks = {}
...
/**
 * Set hooks for CompetitionTask routes
 */
export function setCompetitionTaskHooks(h: CompetitionTaskHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["title", "description", "priority", "category", "status", "dueDay", "owner", "blockedBy", "completedAt", "createdAt", "updatedAt"] as const
...
/**
 * Create CompetitionTask CRUD routes
 */
export function createCompetitionTaskRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        prisma.competitionTask.findMany({
          where,
          include,
          orderBy,
        ...
        prisma.competitionTask.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[CompetitionTask] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.competitionTask.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "CompetitionTask not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[CompetitionTask] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.competitionTask.create({
        data: picked.data,
      })
      ...
      console.error("[CompetitionTask] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.competitionTask.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[CompetitionTask] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.competitionTask.delete({
        where: { id },
      })
      ...
      console.error("[CompetitionTask] Delete error:", error)
