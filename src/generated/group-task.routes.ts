...
/**
 * Auto-generated GroupTask Routes
 *
...
import type { GroupTaskHooks } from "./group-task.hooks"
...
/**
 * Set hooks for GroupTask routes
 */
export function setGroupTaskHooks(h: GroupTaskHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "title", "description", "assignedTo", "priority", "status", "dueDate", "category", "createdAt", "updatedAt"] as const
...
/**
 * Create GroupTask CRUD routes
 */
export function createGroupTaskRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[GroupTask] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupTask.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupTask not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupTask] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupTask.create({
        data: picked.data,
      })
      ...
      console.error("[GroupTask] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupTask.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[GroupTask] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[GroupTask] Delete error:", error)
