...
/**
 * Auto-generated ContractRecord Routes
 *
...
import type { ContractRecordHooks } from "./contract-record.hooks"
...
/**
 * Set hooks for ContractRecord routes
 */
export function setContractRecordHooks(h: ContractRecordHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["jurisdictionCode", "contractType", "scope", "parties", "clauses", "riskFlags", "status", "lastReviewed", "createdAt"] as const
...
/**
 * Create ContractRecord CRUD routes
 */
export function createContractRecordRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[ContractRecord] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.contractRecord.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "ContractRecord not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[ContractRecord] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.contractRecord.create({
        data: picked.data,
      })
      ...
      console.error("[ContractRecord] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.contractRecord.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[ContractRecord] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[ContractRecord] Delete error:", error)
