...
import type { SignoffHooks } from "./signoff.hooks"
...
/**
 * Set hooks for Signoff routes
 */
export function setSignoffHooks(h: SignoffHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["residentId", "jurisdiction", "decision", "signOff", "rowHash", "reviewer", "note", "createdAt"] as const
...
/**
 * Create Signoff CRUD routes
 */
export function createSignoffRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.signoff.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[Signoff] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.signoff.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Signoff not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[Signoff] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.signoff.create({
        data: picked.data,
      })
      ...
      console.error("[Signoff] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.signoff.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[Signoff] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[Signoff] Delete error:", error)
