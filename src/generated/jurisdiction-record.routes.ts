...
/**
 * Auto-generated JurisdictionRecord Routes
 *
...
import type { JurisdictionRecordHooks } from "./jurisdiction-record.hooks"
...
let hooks: JurisdictionRecordHooks = {}
...
/**
 * Set hooks for JurisdictionRecord routes
 */
export function setJurisdictionRecordHooks(h: JurisdictionRecordHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["code", "name", "tenureSystem", "registryName", "registryUrl", "status", "conviction", "lastReviewed", "createdAt"] as const
...
/**
 * Create JurisdictionRecord CRUD routes
 */
export function createJurisdictionRecordRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.jurisdictionRecord.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[JurisdictionRecord] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.jurisdictionRecord.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "JurisdictionRecord not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[JurisdictionRecord] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.jurisdictionRecord.create({
        data: picked.data,
      })
      ...
      console.error("[JurisdictionRecord] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.jurisdictionRecord.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[JurisdictionRecord] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.jurisdictionRecord.delete({
        where: { id },
      })
      ...
      console.error("[JurisdictionRecord] Delete error:", error)
