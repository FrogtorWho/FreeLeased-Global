...
/**
 * Auto-generated GroupBallot Routes
 *
...
import type { GroupBallotHooks } from "./group-ballot.hooks"
...
/**
 * Set hooks for GroupBallot routes
 */
export function setGroupBallotHooks(h: GroupBallotHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["voteId", "option", "memberId", "castAt"] as const
...
/**
 * Create GroupBallot CRUD routes
 */
export function createGroupBallotRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.groupBallot.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[GroupBallot] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupBallot.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupBallot not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupBallot] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupBallot.create({
        data: picked.data,
      })
      ...
      console.error("[GroupBallot] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupBallot.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[GroupBallot] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.groupBallot.delete({
        where: { id },
      })
      ...
      console.error("[GroupBallot] Delete error:", error)
