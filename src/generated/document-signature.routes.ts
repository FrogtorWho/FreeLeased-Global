...
/**
 * Auto-generated DocumentSignature Routes
 *
...
import type { DocumentSignatureHooks } from "./document-signature.hooks"
...
let hooks: DocumentSignatureHooks = {}
...
/**
 * Set hooks for DocumentSignature routes
 */
export function setDocumentSignatureHooks(h: DocumentSignatureHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["documentId", "memberId", "displayName", "signedAt", "ipAddress", "method"] as const
...
/**
 * Create DocumentSignature CRUD routes
 */
export function createDocumentSignatureRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[DocumentSignature] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.documentSignature.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "DocumentSignature not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[DocumentSignature] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.documentSignature.create({
        data: picked.data,
      })
      ...
      console.error("[DocumentSignature] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.documentSignature.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[DocumentSignature] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[DocumentSignature] Delete error:", error)
