...
/**
 * Auto-generated GroupVote Routes
 *
...
import type { GroupVoteHooks } from "./group-vote.hooks"
...
/**
 * Set hooks for GroupVote routes
 */
export function setGroupVoteHooks(h: GroupVoteHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "title", "description", "type", "status", "threshold", "createdBy", "closesAt", "result", "createdAt", "updatedAt"] as const
...
/**
 * Create GroupVote CRUD routes
 */
export function createGroupVoteRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.groupVote.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[GroupVote] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupVote.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupVote not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupVote] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupVote.create({
        data: picked.data,
      })
      ...
      console.error("[GroupVote] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupVote.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[GroupVote] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[GroupVote] Delete error:", error)
