...
/**
 * Auto-generated GroupDocument Routes
 *
...
import type { GroupDocumentHooks } from "./group-document.hooks"
...
/**
 * Set hooks for GroupDocument routes
 */
export function setGroupDocumentHooks(h: GroupDocumentHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "title", "docType", "content", "fileUrl", "uploadedBy", "createdAt"] as const
...
/**
 * Create GroupDocument CRUD routes
 */
export function createGroupDocumentRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        ...
        prisma.groupDocument.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[GroupDocument] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupDocument.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupDocument not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupDocument] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupDocument.create({
        data: picked.data,
      })
      ...
      console.error("[GroupDocument] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupDocument.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[GroupDocument] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[GroupDocument] Delete error:", error)
