...
/**
 * Auto-generated GroupMessage Routes
 *
...
import type { GroupMessageHooks } from "./group-message.hooks"
...
/**
 * Set hooks for GroupMessage routes
 */
export function setGroupMessageHooks(h: GroupMessageHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "memberId", "content", "pinned", "createdAt"] as const
...
/**
 * Create GroupMessage CRUD routes
 */
export function createGroupMessageRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[GroupMessage] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupMessage.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupMessage not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupMessage] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupMessage.create({
        data: picked.data,
      })
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupMessage.update({
        where: { id },
        data: picked.data,
      })
