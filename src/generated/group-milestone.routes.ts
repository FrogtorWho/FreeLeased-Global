...
/**
 * Auto-generated GroupMilestone Routes
 *
...
import type { GroupMilestoneHooks } from "./group-milestone.hooks"
...
/**
 * Set hooks for GroupMilestone routes
 */
export function setGroupMilestoneHooks(h: GroupMilestoneHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["groupId", "title", "description", "deadline", "status", "createdAt", "updatedAt"] as const
...
/**
 * Create GroupMilestone CRUD routes
 */
export function createGroupMilestoneRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      const [items, total] = await Promise.all([
        prisma.groupMilestone.findMany({
          where,
          include,
          orderBy,
        ...
        prisma.groupMilestone.count({ where }),
      ])

      return sendJson(c, { ok: true, items, total })
    } catch (error: any) {
      console.error("[GroupMilestone] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupMilestone.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "GroupMilestone not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[GroupMilestone] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.groupMilestone.create({
        data: picked.data,
      })
      ...
      console.error("[GroupMilestone] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.groupMilestone.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[GroupMilestone] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      await prisma.groupMilestone.delete({
        where: { id },
      })
      ...
      console.error("[GroupMilestone] Delete error:", error)
