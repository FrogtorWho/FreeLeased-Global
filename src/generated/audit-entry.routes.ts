...
/**
 * Auto-generated AuditEntry Routes
 *
...
import type { AuditEntryHooks } from "./audit-entry.hooks"
...
/**
 * Set hooks for AuditEntry routes
 */
export function setAuditEntryHooks(h: AuditEntryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["residentId", "jurisdiction", "agent", "ds", "band", "abstain", "rowHash", "createdAt"] as const
...
/**
 * Create AuditEntry CRUD routes
 */
export function createAuditEntryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AuditEntry] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.auditEntry.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AuditEntry not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AuditEntry] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.auditEntry.create({
        data: picked.data,
      })
      ...
      console.error("[AuditEntry] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.auditEntry.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[AuditEntry] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AuditEntry] Delete error:", error)
