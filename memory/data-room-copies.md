...
| COPY-090 | 2026-08-11T13:27:00Z | (workspace-only — NOT copied to Data Room) | `src/core/ollygarden_observability.py` (edited — `AUTH_HEADER_NAME = "Authorization"` + `AUTH_HEADER_SCHEME = "Bearer"`; line 160 sends `Bearer <key>`) | n/a | Phase 7+ — OllyGarden wire-format fix. The Mavis-canonical wrapper sent `X-OllyGarden-Key: <key>` (verbatim); the partner collector only accepts `Authorization: Bearer <key>`. Proof probe (`scripts/proof-probe-endpoints.py`) confirmed Bearer advances 401 → 400 (auth now passes). All keys masked (`og_s***`). | revert commit | OK (HTTP 400 post-fix in 216 ms; auth verified) |
| COPY-091 | 2026-08-11T13:27:00Z | (workspace-only — NOT copied to Data Room) | `src/test_ollygarden.py` (edited — line 19 Bearer) + `scripts/activate-ollygarden-live.py` (edited — line 111 Bearer; lines 54-69 trace-id length fix) | n/a | Phase 7+ — apply the Bearer fix to the v1 test harness + the live activation script. Body-shape follow-up: the activate script's `to_otlp()` was rjust'ing ascii trace_ids then hex-encoding, producing 64 hex chars (twice OTLP spec). Replaced with hex-aware normalisation (zero-pad if hex, sha256-truncate if ascii). All keys masked (`og_s***`). | revert commit | OK (HTTP 200 in 212 ms; trace accepted) |
| COPY-092 | 2026-08-11T13:27:00Z | (workspace-only — NOT copied to Data Room) | `docs/ollygarden-integration.md` (edited — new "Wire format (auth header)" section §5; section 9 honest disclosure updated) + `project/strategy/live-activation-proof.md` (edited — verdict flipped PARTIAL → LIVE in summary table; "Conflict in source code" block retitled "FIX APPLIED"; verdict retitled "FIX APPLIED at 13:20 UTC"; F1 retitled "FIX APPLIED at 13:20 UTC") | n/a | Phase 7+ — docs + proof update. Wire format table in the manual now lists all 5 live senders and confirms Bearer is canonical. Proof doc's "Conflict in source code" block is retitled to reflect the fix; the F1 entry in the new-facts section records the action taken (Bearer + trace-id) with line refs. | revert commit | OK |
| COPY-093 | 2026-08-11T13:27:00Z | (workspace-only — NOT copied to Data Room) | `memory/2026-08-11-ollygarden-sample.json` (edited — overwrote pre-fix 401 record with post-fix 200 record; same path, new content) | n/a | Phase 7+ — OllyGarden activation artefact now reflects the post-fix HTTP 200 in 212 ms with response body `{"partialSuccess":{}}`. The pre-fix 401 record was overwritten on disk; the proof doc + AI_JOURNAL preserve the 401 history. All keys masked (`og_s***`). | revert commit | OK (HTTP 200; `partialSuccess:{}` captured) |
| COPY-094 | 2026-08-11T13:27:00Z | (workspace-only — NOT copied to Data Room) | `scripts/probe-ollygarden-body-shape.py` + `scripts/probe-ollygarden-traceid.py` (both new) + `memory/2026-08-11-ollygarden-body-probe.json` + `memory/2026-08-11-ollygarden-traceid-probe.json` (both new) + `HEARTBEAT.md` (edited — 13:27 UTC bullet) + `AI_JOURNAL.md` (edited — new "OllyGarden wire-format fix" section) | n/a | Phase 7+ — diagnostic probes + journal entries. The differential trace-id probe conclusively identified the secondary bug (trace_id length must be exactly 32 hex chars). All keys masked (`og_s***`). | revert commit | OK |
